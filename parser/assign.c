...
typedef void (*t_assign_fn)(const t_option *options, char *value,
	t_parser_ctx *ctx);
...
static void assign_boolean(const t_option *options, char *value,
	t_parser_ctx *ctx);
static void assign_int(const t_option *options, char *value, t_parser_ctx *ctx);
static void assign_uint(const t_option *options, char *value,
	t_parser_ctx *ctx);
static void assign_double(const t_option *options, char *value,
	t_parser_ctx *ctx);
static void assign_string(const t_option *options, char *value,
	t_parser_ctx *ctx);
static void assign_count(const t_option *options, char *value,
	t_parser_ctx *ctx);
static void assign_callback(const t_option *options, char *value,
	t_parser_ctx *ctx);

/**
 * @brief Assigns an option's value to its typed target.
 *
 * Based on the type flags (TYPE_*), this function converts the string value and
 * writes the result into options->value, or triggers a specific behavior
 * (counter, callback, boolean). Supported types: - TYPE_BOOLEAN : sets the
 * target value to true, ignores value. - TYPE_INT : parses a signed integer. -
 * TYPE_UINT : parses an unsigned integer. - TYPE_DOUBLE : parses a floating
 * point number. - TYPE_STRING : stores the raw pointer to value. - TYPE_COUNT :
 * increments an integer counter. - TYPE_CALLBACK : calls the function stored in
 * options->value. On parsing error or missing value, err is updated with the
...
static void assign_int(const t_option *options, char *value,
	t_parser_ctx *ctx) {
	if (!value) {
	...
	/**
	 * Smart saturation We handle the case where strtol itself overflow (ERANGE)
	 * and the case where the length is valid but too large or too small for an
	 * int.
	 */
	if (errno == ERANGE) {
		if (result > 0)
...
static void assign_uint(const t_option *options, char *value,
	...
	/**
	 * Smart saturation If strtoul detects an overflow (ERANGE) or if result
