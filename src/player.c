 ...
 *
 * To guarantee perfect uniform distribution without endless guessing loops, it
...
static int player_place(t_data *data, t_pos *pos) {
	...
	// 3. Register the player's team on the board and convert 1D index to 2D
	// coordinates
	board[i] = data->team;
...
bool player_move(t_data *data, t_pos *pos, int dx, int dy) {
	...
	log_push(data, "[>] Team %d moved from (%d, %d) to (%d, %d)", data->team,
		pos->x, pos->y, nx, ny);
