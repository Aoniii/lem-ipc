...
t_validate validate_team(t_data *data, char **args) {
	...
	// Error if characters remain unparsed (*endptr) or no digits were evaluated
	// entirely
...
/**
 * @brief Verifies that the designated AI difficulty level matches authorized
 * rules.
 */
...
/**
 * @brief Validates file accessibility, system permissions, and naming layout
 * constraints.
 *
 * Uses `access(..., R_OK)` to verify file readability before starting the
 * replay loop.
 */
...
static const char *validate_msg[] = {
	...
	[V_INVALID_AI_LEVEL] =
		"AI level must be between " STR(MIN_AI_LEVEL) " and " STR(MAX_AI_LEVEL),
	[V_INVALID_MAP_SIZE] =
		"map size must be between " STR(MIN_MAP_SIZE) " and " STR(MAX_MAP_SIZE),
	[V_CONFLICT_SPECTATOR_REPLAY] =
		"--spectator and --replay are mutually exclusive",
	[V_CONFLICT_SPECTATOR_HUMAN] =
		"--spectator and --human are mutually exclusive",
...
/**
 * @brief Converts internal evaluation enumeration flags into user-facing text
 * strings.
 */
