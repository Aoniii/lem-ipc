 ...
 *
 * Scans the board and gathers coordinates of all players belonging to the same
 * team.
 */
 ...
 * players of the same enemy team. Evaluates all 8 neighboring directions
...
bool is_circled(t_data *data, t_pos *pos) {
	...
	while (i < 8) {
		...
		if (tile != TILE_WALL) {
			counts[tile]++;
			// Core Rule: At least 2 players from the SAME enemy team are
			// adjacent
			if (counts[tile] >= 2) {
				header->kill[tile]++;
				return (true);
 ...
 *
 * Iterates through the board to count how many distinct teams still have active
 * agents.
 */
 ...
 * This guarantees that all empty paths generated are contiguous (no isolated
 * pockets).
 */
void walls_generator(t_data *data) {
	...
	while (free_tile > 0) {
		...
		while (!new_direction) {
			...
			if (x + dx[rdm] < 0 || x + dx[rdm] >= (int)data->map_size ||
				y + dy[rdm] < 0 || y + dy[rdm] >= (int)data->map_size)
				continue;
