 ...
 * colors (solid blocks). Phase 2: Generates up to 15 unique combinations of
 * foreground/background mixes to support up to 21 distinct teams visual IDs.
 */
...
/**
 * @brief Initializes the ncurses environment, locale settings, and window
 * safety.
 *
 * Sets wide-character support (UTF-8), configures terminal behavior,
...
int display_init(t_data *data) {
	...
	// Non-blocking keyboard input configurations
	nodelay(stdscr, TRUE);
	return (0);
}

/**
 * @brief Draws a box outline using standard ncurses ACS (Alternate Character
 * Set) symbols.
 */
...
/**
 * @brief Draws the board centered in the frame. Cells are 2 chars wide so they
 * look square. The local player blinks.
 */
...
static void draw_logs(t_data *data, unsigned int start_y) {
	...
	while (i < header->log_count) {
		idx = (header->log_head - header->log_count + i + LOG_COUNT) %
			LOG_COUNT;
...
static void draw_team_info(t_data *data, unsigned int start_y) {
	...
	// Print custom colorful square blocks
	addstr("\u2580\u2584");
...
/**
 * @brief Safely shuts down the ncurses context and restores original terminal
 * settings.
 */
