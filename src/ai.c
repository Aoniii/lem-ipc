 ...
 *
 * Uses a Breadth-First Search (BFS) algorithm to locate the closest valid
 * target from its own position, then takes a single step toward it.
 */
 ...
 *
 * A target is valid if it lies within the map boundaries, is not empty space or
 * a wall, and belongs to an opposing team.
 */
static bool target_is_valid(t_data *data, t_pos target) {
	unsigned char tile;

	// Map boundaries check
	if (target.x < 0 || target.x >= (int)data->map_size || target.y < 0 ||
		target.y >= (int)data->map_size)
		return (false);
 ...
 *
 * 1. Checks if a teammate broadcasted a target via messages. If valid, the AI
 * pursues it and relays the message. 2. Otherwise, runs a multi-source BFS
 * using the entire team's coordinates to find the nearest enemy, moves toward
 * them, and broadcasts the new target.
 */
static void ai_coordination_move(t_data *data, t_pos *pos) {
	...
	if (res.found) {
		move = ai_step_to(data, *pos, res.target);
		player_move(data, pos, move.x, move.y);
		// Share the found target with the team
 ...
 *
 * Dispatches the entity to the appropriate movement routine based on its
 * configuration.
 */
