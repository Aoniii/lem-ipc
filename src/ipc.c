 ...
 *
 * ftok() needs an existing pathname to generate the IPC key, so we guarantee
 * the file exists before calling it.
 */
 ...
 *
 * Exclusively creates the SHM, semaphore and message queue, zeroes the board,
 * writes the header metadata, then sets ready = 1 last so joiners only read
...
static int ipc_create(t_data *data, key_t key) {
	...
	// signal joiners that init is done
	header->ready = 1;
	data->is_first = true;
	return (0);
}
 ...
 *
 * Joining processes spin on the ready flag before reading map_size, so they
 * never read uninitialized data.
 */
 ...
 *
 * Looks up the SHM, semaphore and message queue without modifying them, then
 * reads the board configuration from the header.
 */
 ...
 *
 * Ensures the token file exists, tries to create the resources as the first
 * player, and falls back to attaching if a game already exists.
 */
 ...
 *
 * Decrements the shared player count under the semaphore. The last one out
 * removes the SHM, semaphore, message queue and token file.
 */
void ipc_cleanup(t_data *data) {
	...
	if (last) {
		// Remove the SHM
		shmctl(data->shm_id, IPC_RMID, NULL);
		// Wipe Semaphore Array
		semctl(data->sem_id, 0, IPC_RMID);
		// Wipe Message Queue
		msgctl(data->msg_id, IPC_RMID, NULL);
		// Wipe identification link file
		unlink(IPC_PATH);
	}
}
