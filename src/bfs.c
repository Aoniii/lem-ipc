 ...
 *
 * Explores the map starting from multiple source positions simultaneously
 * (e.g., all teammates). It stops and returns the coordinates of the first
 * valid enemy target encountered.
 */
t_bfs_result ai_bfs_multi(t_data *data, t_pos *sources, int count) {
	...
	// 3. Push all starting sources into the queue to perform multi-source
	// exploration
	i = 0;
	while (i < count) {
	...
	while (head < tail) {
		...
		while (i < 4) {
			...
			// Boundary check
			if (nx < 0 || nx >= (int)data->map_size || ny < 0 ||
				ny >= (int)data->map_size)
				continue;
			...
			// Target found: Tile is not empty, and does not belong to our team
			// (Enemy)
...
/**
 * @brief Backtracks from the destination tile to find the very first step to
 * take.
 *
...
static void build_dir_order(t_pos src, t_pos dest, int *order) {
	...
	// Choose specific direction indices based on sign: Up(0), Down(1), Left(2),
	// Right(3)
	h = (ddx > 0) ? 3 : 2;
	v = (ddy > 0) ? 1 : 0;

	// Prioritize horizontal or vertical exploration depending on which distance
	// is greater
	if (abs(ddx) >= abs(ddy)) {
		order[0] = h;
		order[1] = v;
...
/**
 * @brief Standard BFS pathfinding to compute the next single step towards a
 * destination.
 *
