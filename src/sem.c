 ...
 *
 * Creating a semaphore and setting its value are two separate calls, so joiners
 * wait for sem_otime to become non-zero (set by the creator's first semop)
 * before using it.
 */
static int sem_wait_ready(int sem_id) {
	...
	while (retry < SEM_MAX_RETRY) {
		...
		// sem_otime turns non-zero exactly when the first semop() is
		// successfully committed
		if (buf.sem_otime != 0)
			return (0);

		// Back off to preserve CPU overhead
		usleep(1000);
		retry++;
	}
	return (-1);
 ...
 *
 * Sets the value to 0, then a +1 semop brings it to 1 and sets sem_otime (which
 * is what joiners wait on).
 */
 ...
 *
 * Decrements the semaphore; blocks if it's already at 0. SEM_UNDO releases the
 * lock automatically if the process dies while holding it.
 */
void sem_lock(int sem_id) {
	...
	op.sem_num = 0;
	op.sem_op = -1;
	// Auto-cleanup tracking flag activated
	op.sem_flg = SEM_UNDO;
