...
int main(int argc, char **argv) {
	...
	t_option options[] = {
		{
			...
			.value = &data.map_size,
			.help =
				"set map size (first player only, default: " STR(
				DEFAULT_MAP_SIZE) ")"
		},
		{
			.short_opt = 0,
			...
			.value = &data.ai,
			.help =
				"set AI level: 1=random, 2=chase, 3=coordinated (default: " STR(
				DEFAULT_AI_LEVEL) ")"
		},
		{
			.short_opt = 0,
	...
	// 2. Perform validation logic matching on unmapped remaining arguments
	// (e.g., Team ID)
	...
	// 3. Setup system interception hooks (SIGINT, SIGTERM, etc.) for graceful
	// shutdown
	setup_signals();
