 ...
 *
 * Same rendering as the game (walls, empty, team half-blocks) but reads from
 * the replay's reconstructed board.
 */
...
void display_render_replay(t_replay *replay) {
	...
	mvprintw(replay->map_size + 2, 2, "%s   time: %ld ms",
		replay->playing ? "[PLAYING]" : "[PAUSED] ", replay->ms_saved);
	mvprintw(replay->map_size + 3, 2,
