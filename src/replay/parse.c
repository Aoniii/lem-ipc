 ...
 *
 * Validates the size and that the board string only contains 0/1, then fills
 * map_size and initial_board.
 */
 ...
 *
 * Format: <ms> <id> <TYPE> <team> <x> <y> [<dx> <dy>] Checks ms is
 * non-decreasing, id increments by 1, the type is known, coordinates are in
 * bounds, and (for MOVE) the direction is one step.
 */
static int parse_event(t_replay *replay, char *line, long *last_ms,
	...
	// in bounds
	if (ev->x < 0 || ev->x >= (int)replay->map_size || ev->y < 0 ||
		ev->y >= (int)replay->map_size) {
		free(ev);
		return (-1);
	}
...
int replay_parse(t_replay *replay, int fd) {
	...
	ft_memcpy(replay->board, replay->initial_board,
		replay->map_size * replay->map_size);
